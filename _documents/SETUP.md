...
http://localhost:3000 （ポート指定時は http://localhost:4040 ）を開いて以下を確認:

- **ヘッダー**: 「ララチューン」タイトルと「ラランド公式 YouTube Analytics」サブタイトルが表示される
...
> 推移グラフはデータが2日分以上たまると線グラフになる。数日間 GitHub Actions で自動収集を続けると意味のあるグラフになる。
